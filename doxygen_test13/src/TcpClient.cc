...
TcpClient::~TcpClient(){
    ...
    if(conn){
        // printf("this is detail::removeConnection\n");
...
void TcpClient::newConnection(int sockfd){
    ...
    InetAddress peerAddr(sockets::getPeerAddr(sockfd)); //获取连接对端的IP地址
    char buf[32];
    snprintf(buf, sizeof buf, ":%s#%d", peerAddr.toHostPort().c_str(), nextConnId_);
    ++nextConnId_;
    string connName = buf;

    InetAddress localAddr(sockets::getLocalAddr(sockfd)); //获取自身的IP地址
...
void TcpClient::removeConnection(const TcpConnectionPtr& conn){
    // printf("this is TcpClient::removeConnection\n");
    loop_->assertInLoopThread();
    ...
    if(retry_ && connect_){
        LOG_INFO <<  "TcpClient::connect[" << "] - Reconnecting to " << connector_->serverAddress().toHostPort();
        connector_->restart();
    }
}