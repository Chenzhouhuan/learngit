...
void MimeType::init() { //初始化各种mime类型
  mime[".html"] = "text/html";
...
std::string MimeType::getMime(const std::string &suffix) {
  pthread_once(&once_control, MimeType::init); //pthread_once静态初始化，只在本进程执行序列中执行一次
...
HttpData::HttpData(EventLoop *loop, int connfd)
    : loop_(loop),
      ...
      method_(METHOD_GET), //GET或者POST
      HTTPVersion_(HTTP_11), //使用的Http版本
      nowReadPos_(0),
      state_(STATE_PARSE_URI),
      hState_(H_START),
      keepAlive_(false) {
  // loop_->queueInLoop(bind(&HttpData::setHandlers, this));
  channel_->setReadHandler(bind(&HttpData::handleRead, this)); //向管道注册handleRead()函数
  channel_->setWriteHandler(bind(&HttpData::handleWrite, this)); //向管道注册handleWrite()函数
  channel_->setConnHandler(bind(&HttpData::handleConn, this)); //向管道注册handleConn()函数
}

void HttpData::reset() { //重置
  // inBuffer_.clear();
  fileName_.clear();
  path_.clear();
  nowReadPos_ = 0;
...
void HttpData::handleRead() { //被回调
  __uint32_t &events_ = channel_->getEvents();
  do {
    bool zero = false;
    int read_num = readn(fd_, inBuffer_, zero); //为什么传入bool参数?
    ...
    if (state_ == STATE_PARSE_URI) {
      URIState flag = this->parseURI(); //解析URI
      if (flag == PARSE_URI_AGAIN)
        ...
        inBuffer_.clear();
        error_ = true;
        handleError(fd_, 400, "Bad Request(flag == PARSE_URI_ERROR)");
        break;
      } else
        state_ = STATE_PARSE_HEADERS;
    }
    if (state_ == STATE_PARSE_HEADERS) {
      HeaderState flag = this->parseHeaders(); //解析头部
      if (flag == PARSE_HEADER_AGAIN)
        break;
      else if (flag == PARSE_HEADER_ERROR) {
        perror("3");
        error_ = true;
        handleError(fd_, 400, "Bad Request(flag == PARSE_HEADER_ERROR)");
        break;
      }
...
URIState HttpData::parseURI() { //解析URL
  ...
  if (pos < 0) {
    ...
    size_t _pos = request_line.find(' ', pos); //去除空格行
    if (_pos < 0)
      return PARSE_URI_ERROR;
    else {
      if (_pos - pos > 1) {
  ...
  // cout << "fileName_: " << fileName_ << endl; //打印结果看看
  // HTTP 版本号
  pos = request_line.find("/", pos);
  if (pos < 0)
    return PARSE_URI_ERROR;
...
HeaderState HttpData::parseHeaders() { //解析头
  string &str = inBuffer_;
    ...
    switch (hState_) {
      ...
      case H_CR: {
        if (str[i] == '\n') {
          ...
          //打印测试查看
          // cout << "HeaderState HttpData::parseHeaders()-->key: " << key << endl;
          // cout << "HeaderState HttpData::parseHeaders()-->value: " << value << endl;
          headers_[key] = value;
          now_read_line_begin = i;
        } else
          return PARSE_HEADER_ERROR;
...
AnalysisState HttpData::analysisRequest() {
  if (method_ == METHOD_POST) {
    ...
    if (stat(fileName_.c_str(), &sbuf) < 0) {
      header.clear();
      handleError(fd_, 404, "Not Found!(stat(fileName_.c_str(), &sbuf) < 0)");
    ...
    if (src_fd < 0) {
      outBuffer_.clear();
      handleError(fd_, 404, "Not Found!(src_fd < 0)");
      return ANALYSIS_ERROR;
    }
    ...
    if (mmapRet == (void *)-1) {
      ...
      outBuffer_.clear();
      handleError(fd_, 404, "Not Found!(mmapRet == (void *)-1)");
      return ANALYSIS_ERROR;
    }
